...
def load_german_sequences(
    ...
    with open(annotations_csv, newline="") as f:
        ...
        for row in reader:
            patient_no_raw = row["PatientNo"].strip()
            case_id = row["CaseID"].strip()
            gt_str = row["Ground truth"].strip().lower()

            if not patient_no_raw or not case_id:
                continue

            try:
                patient_no_int = int(patient_no_raw)
            except ValueError:
                # If PatientNo is not numeric, skip
                continue

            patient_no = f"{patient_no_int:03d}"
            patient_name, phase = _parse_case_id(case_id)  # e.g. ('wen_ro_m50', 'pre')
            label = THROMBUS_YES if gt_str == "true" else THROMBUS_NO

            patient_folder = f"{patient_no}-{patient_name}"
            ...
            if frontal_folder is None or lateral_folder is None:
                # Require both views
                print(f"[GermanData] Skipping {patient_folder} ({phase}): missing frontal or lateral folder.")
                continue
            ...
            if frontal_nii is None or lateral_nii is None:
                print(f"[GermanData] Skipping {patient_folder} ({phase}): missing .nii in view folder.")
                continue

            seq_id = f"{patient_no}-{patient_name}-{phase}"
            sequences[seq_id] = {
